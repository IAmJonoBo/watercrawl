...
class CircuitBreaker:
    """Simple circuit breaker for rate-limit and connection errors."""
...
class FirecrawlClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._state = {
            ...
            "min_interval": self._safe_float(max(
    ...
    def await_rate_limit(self) -> None:
        ...
        if type(lock) is threading.Lock:
            with lock:
                ...
                if remaining > 0:
                    logger.info("Waiting %.2fs for Firecrawl rate-limit reset", remaining)
    ...
    def scrape(self, url: str, *, formats: Optional[List[Any]] = None) -> Any:
        scrape_formats = formats or list(self._settings.behaviour.scrape_formats)

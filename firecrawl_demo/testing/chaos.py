...
class ChaosOrchestrator:
    ...
    def game_day(self, scenario_id: str) -> Generator[GameDayResult, None, None]:
        ...
        """
        result = GameDayResult(scenario_id=scenario_id, start_time=datetime.now())
    ...
    def inject_failure(
        self, mode: FailureMode, component: str = "", duration_s: Optional[float] = None
    ) -> FailureInjection:
        """
        ...
        if not self.config.enable_chaos:
            ...
            return FailureInjection(
                mode=mode, start_time=datetime.now(), affected_component=component
            )
        ...
        if (
            self.config.allowed_failure_modes
            and mode not in self.config.allowed_failure_modes
        ):
        ...
        is_sensitive_component = any(
            kw in component.lower() for kw in sensitive_keywords
        )
        if is_sensitive_mode or is_sensitive_component:
            logger.warning(
                "Injected a sensitive failure mode (details omitted for security)."
            )
        else:
    ...
    def recover_failure(self, injection: FailureInjection) -> None:
        ...
        injection.duration_s = (
            injection.end_time - injection.start_time
        ).total_seconds()
        ...
        logger.info(
            f"Recovered from {injection.mode.value} after {injection.duration_s}s"
        )

    @contextmanager
    def inject_transient_failure(
        self, mode: FailureMode, component: str = ""
...
def execute_game_day_scenario(
    scenario_id: str, orchestrator: Optional[ChaosOrchestrator] = None
) -> GameDayResult:
    """

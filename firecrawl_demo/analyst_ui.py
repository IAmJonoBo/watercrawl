...
def main():
    ...
    idx = st.number_input("Row index to annotate", min_value=0, max_value=len(df)-1, step=1)
    feedback = st.text_area("Feedback/Flag")
    if st.button("Save Feedback"):
        prev = df.at[idx, "Analyst Feedback"] if "Analyst Feedback" in df.columns else ""
        ...
        with audit_path.open("a", encoding="utf-8", newline="") as fh:
            writer = csv.DictWriter(fh, fieldnames=["Timestamp", "RowIndex", "Previous", "New"])
            if not file_exists:
                writer.writeheader()

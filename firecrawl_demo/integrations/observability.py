...
class SLOMetrics:
    ...
    def add_request(self, success: bool, latency_ms: float, target_latency_ms: float) -> None:
...
class ObservabilityManager:
    ...
    def _init_tracing(self, resource: Resource) -> None:
        ...
        logger.info(f"Tracing initialized with OTLP endpoint: {self.config.otlp_endpoint}")
    ...
    def _init_metrics(self, resource: Resource) -> None:
        ...
        logger.info(f"Metrics initialized with Prometheus on port {self.config.prometheus_port}")
    
    def _observe_slo(self, options) -> None:
        """Callback to observe SLO metrics."""
        yield metrics.Observation(
            self.slo_metrics.availability(),
            {"sli": "availability"}
        )
        yield metrics.Observation(
            self.slo_metrics.latency_sli(),
            {"sli": "latency"}
        )
        yield metrics.Observation(
    ...
    def _record_request(
        ...
        if self.config.enable_slos:
            self.slo_metrics.add_request(success, latency_ms, self.config.target_latency_ms)
        
        # Record to metrics instruments
        if self._request_counter:
            self._request_counter.add(1, {"operation": operation, "success": str(success)})
    ...
    def get_slo_status(self) -> dict:
        ...
        compliance = {
            "availability_ok": metrics_dict["availability"] >= self.config.target_availability,
            "latency_ok": metrics_dict["latency_sli"] >= self.config.target_availability,
            "error_rate_ok": metrics_dict["error_rate"] <= self.config.target_error_rate,
        }

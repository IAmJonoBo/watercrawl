...
- [x] 2025-10-22 — Row processing refactor: Extracted transformation logic into `row_processing.py` and `change_tracking.py` modules, replaced `.iterrows()` with `.itertuples()` for 2-3x iteration speedup, added bulk DataFrame updates to avoid repeated dtype conversions, created system integration tests (`tests/system/`), contract consumer tests, and performance smoke tests with CI gating, updated CI workflow with new test stages (unit, contract, performance, system), published QA runbooks in `docs/qa.md`, updated `docs/operations.md` with SIT/contract gate descriptions and coverage delta reporting.

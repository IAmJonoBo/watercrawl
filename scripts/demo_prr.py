...
def demo_prr():
    ...
    categories = [
        ("Quality & Functionality", [
            ("Unit/Integration/E2E Tests", "Pass", "Tests configured in pyproject.toml"),
            ("Coverage Thresholds", "Pass", "Coverage tool configured"),
            ("Linting Configuration", "Pass", "Linters configured: ruff, black, isort"),
            ("Static Analysis", "Pass", "Static analysis tools configured: mypy, bandit"),
        ]),
        ...
        ("Security & Privacy", [
            ...
            ("Secrets/Least Privilege", "Pass", "Secrets management tools: .env.example template, bandit"),
            ("SAST/DAST/Dependency Scans", "Pass", "Security scanning tools: bandit (SAST), safety (dependency scan)"),
            ("Vulnerability SLAs", "Pass", "Vulnerability response SLAs documented in SECURITY.md"),
            ("Data Protection & Retention", "Pass", "Found 0 docs, 2 modules"),
        ]),
        ("Supply Chain", [
            ("SBOM (SPDX/CycloneDX)", "Warn", "Lock files present (poetry.lock) but no SBOM"),
            ("Reproducible/Signed Builds", "Pass", "Build automation found: 2 files"),
            ("Provenance Attestation (SLSA)", "Skip", "Skipped (optional check)"),
        ]),
        ("Compliance & Licensing", [
            ("Third-party License Obligations", "Pass", "Project license found: 1 files"),
        ]),
        ("Observability & Ops", [
            ("Metrics/Logs/Traces", "Pass", "Telemetry configured: OpenTelemetry, Prometheus, structlog"),
        ...
        ("Deployment & Change", [
            ...
            ("Config Pinned", "Pass", "Dependencies pinned: poetry.lock, pnpm-lock.yaml"),
            ("Blue/Green or Canary Plan", "Skip", "Skipped (optional check)"),
            ("Schema/Data Migrations", "N/A", "No migration system found (may not be needed)"),
            ("Feature Flags", "Pass", "Feature flags found: 5 files, env_example=yes"),
        ]),
        ("Docs & Comms", [
    ...
    pass_count = sum(1 for _, checks in categories for _, status, _ in checks if status == "Pass")
    fail_count = sum(1 for _, checks in categories for _, status, _ in checks if status == "Fail")
    warn_count = sum(1 for _, checks in categories for _, status, _ in checks if status == "Warn")
    na_count = sum(1 for _, checks in categories for _, status, _ in checks if status == "N/A")
    skip_count = sum(1 for _, checks in categories for _, status, _ in checks if status == "Skip")
    ...
    print("=" * 70)
    print()
    print("Evidence bundle saved: artifacts/prr/evidence/prr_report_20251024T000000Z.json")
    print()
    ...
    print("poetry run python -m apps.automation.cli qa prr --output artifacts/prr/report.json")
    print()
